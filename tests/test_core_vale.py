...
def mock_openai_client():
    """Vale unit tests do not need a real HTTP client or host proxy settings."""
    with patch("docs_style.core_vale.ChatOpenAI"):
        yield
...
def test_vale_fix_applied(mock_llm_cls, mock_run, mock_which, tmp_path):
    ...
    with patch("docs_style.core_vale.ChatPromptTemplate.from_template") as mock_prompt_tmpl:
        ...
        # Replace the composed prompt/model/parser chain.
...
def test_vale_max_retries_reached(mock_llm_cls, mock_run, mock_which, tmp_path):
    ...
    with patch("docs_style.core_vale.ChatPromptTemplate.from_template") as mock_prompt_tmpl:
        mock_chain = Mock()
        mock_chain.invoke.side_effect = ["modified once", "modified twice"]
        mock_prompt_tmpl.return_value.__or__.return_value.__or__.return_value = mock_chain
        ...
        assert f.read_text() == "modified twice"
