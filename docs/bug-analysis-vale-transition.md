...
In `docs_style/tui.py`, the `run_initial_vale_check()` method is decorated with `@work(exclusive=True, thread=True)`, meaning it runs in a background thread. At the end of this method, it was calling:
...
- Fixed: `docs_style/tui.py` (lines 398-421)
