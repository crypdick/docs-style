...
- `docs_style/workflow.py`: environment setup, input resolution, notebook
  pairing, and guide selection; returns `WorkflowContext`. Environment setup
...
- `docs_style/cli.py` and `main.py`: command-line entry points.
  `docs_style/tui_arguments.py` defines the terminal user interface (TUI)
  argument parser.
...
- `bulk_pr_autodocs.py` and `crawl/`: maintenance scripts with their own PEP 723
  dependencies. The bulk script changes Git and GitHub state when invoked.
...
Guide ordering and final-pass selection belong in `workflow.py` and `settings.py`.
The Python editor operates from a checkout; it does not promise a self-contained wheel
