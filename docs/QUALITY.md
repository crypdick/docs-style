...
Coverage grades use these thresholds: A at 90%, B at 80%, C at 60%, D greater than 0%,
...
The `settings.py`, `utils.py`, and `main.py` files pass strict mypy and Ruff but are outside
the package coverage measurement. Ruff checks the crawler and bulk pull request (PR)
...
- Cover quit-button routing in the terminal user interface (TUI), failures when
  writing or syncing documents, and terminal startup.
...
The `htmlcov/index.html` file provides source views. Update these grades after meaningful
...
The three findings in the secrets baseline are literal placeholders in the `.env.SAMPLE`
and `README.md` files, reviewed as non-secrets. The hook scans for additional findings.
